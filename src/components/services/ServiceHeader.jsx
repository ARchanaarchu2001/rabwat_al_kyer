...
const ServiceHeader = ({
  title = "Our Services",
  subtitle = "Explore our professional maintenance and installation services.",
  ...
  useEffect(() => {
    ...
    if (titleRef.current) {
      ...
      titleRef.current.style.transform = `translate3d(${mousePosition.x * 0.1}px, ${yMove}px, 0)`;
    }

    if (subtitleRef.current) {
      const yMove = -scrollY * 0.2 + mousePosition.y * 0.1;
      subtitleRef.current.style.transform = `translate3d(${mousePosition.x * 0.05}px, ${yMove}px, 0)`;
    }
    ...
    floatingShapesRef.current.forEach((shape, index) => {
      if (shape) {
        ...
        shape.style.transform = `translate3d(${xMove}px, ${yMove}px, 0) rotate(${scrollY * 0.1 + index * 45}deg)`;
      }
    });
    ...
    const animateShapes = () => {
      shapes.forEach((shape, index) => {
        if (shape) {
          ...
          const baseTransform = currentTransform.split(' ')[0]; // Keep parallax transform
    ...
    return Array.from({ length: count }, (_, i) => (
      <div
        ...
        className={`absolute opacity-20 transition-all duration-2000 ${
          i % 4 === 0 ? 'bg-[#A2811F] rounded-lg' : 
          i % 4 === 1 ? 'bg-[#9D8A4A] rounded-full' :
          i % 4 === 2 ? 'bg-[#8A7734] triangle' : 'bg-[#A2811F] hexagon'
        }`}
        style={{
          width: `${20 + (i * 5) % 40}px`,
          height: `${20 + (i * 5) % 40}px`,
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <div className="flex flex-col items-center justify-center text-center text-white gap-6">
          ...
          <h1
            ...
            style={{
              background: "linear-gradient(135deg, #ffffff 0%, #F7F1E9 20%, #A2811F 60%, #9D8A4A 100%)",
          ...
          >
            <div className="w-6 h-10 border-2 border-[#A2811F] rounded-full flex justify-center">
              <div className="w-1 h-3 bg-[#A2811F] rounded-full mt-2 animate-bounce" />
            </div>
          </div> */}
        </div>
      </div>
